...
int** hirschberg_profile_alignment (struct alignment* aln, int* tree, float**submatrix, int** map)
{
  ...
  show_percentage_progress("Alignment (Hirschberg Profile)",
                           current_percentage,
                           stdout);
  ...
  for (i = 0; i < (local_numseq - 1); i++)
  {
    ...
    if ((int) ( 1.0 * i / local_numseq * 100) > current_percentage)
    {
      current_percentage = (int) ( 1.0 * i / local_numseq * 100);
      show_percentage_progress("Alignment (Hirschberg Profile)",
                               current_percentage,
                               stdout);
    }
  ...
  show_percentage_end("Alignment (Hirschberg Profile)",
                      stdout);
  free (profile);
  hirsch_mem_free (hm);
...
float** protein_profile_wu_distance (struct alignment* aln, float** dm, struct parameters* param, int nj)
{
  ...
  int current_percentage = 0;
  show_percentage_progress("Distances Calculation (Protein Profile Wi Distance)",
                           current_percentage,
                           stdout);
  ...
  show_percentage_end("Distances Calculation (Protein Profile Wi Distance)", stdout);
...
float** dna_profile_distance (struct alignment* aln, float** dm, struct parameters* param, int nj)
{
  ...
  int current_percentage = 0;
  show_percentage_progress("Distances Calculation (DNA Profile Distance)",
                           current_percentage,
                           stdout);
  ...
  show_percentage_end("Distances Calculation (DNA Profile Distance)", stdout);
