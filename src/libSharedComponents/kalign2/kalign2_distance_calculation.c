...
float** protein_wu_distance (struct alignment  *si,
                             float            **dm,
                             struct parameters *param,
                             int                nj)
{
  struct bignode* hash[1024*1024];
  int*p = 0;
  int i, j, a, b;
  unsigned int hv;
  float min;
  ...
  for (i = 0; i < numseq - 1; i++)
  {
    ...
    for (j = i + 1; j < numseq; j++)
    {
      ...
      //cutoff = param->zlevel;
      p = si->s[j];
      dm[i][j] = protein_wu_distance_calculation (hash,
                                                  p,
                                                  si->sl[j],
                                                  si->sl[j] + si->sl[i],
                                                  cutoff);
...
float protein_wu_distance_calculation (struct bignode *hash[],
                                       const int      *seq,
                                       const int       seqlen,
                                       const int       diagonals,
                                       const float     mode)
