...
int main (int argc, char **argv)
{
  ...
  */
  ...
  int j;
  float **submatrix = malloc (sizeof (float*) * 100);

  for (i = 0; i < 100; i++)
  {
    submatrix[i] = malloc (sizeof (float) * 100);
  ...
  for (i = 0; i < numseq; i++)
  {
    ...
    aln->seq[i] = (int*) malloc (sizeof (int) * (aln->sl[i]+1));

    for (j = 0; j < aln->sl[i]; j++)
    {
      aln->s[i][j]   = 100 - i - j - 1;
      aln->seq[i][j] = 100 - i - j - 1;
